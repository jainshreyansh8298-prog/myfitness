<x-dashboard.main-layout>
    <div class="card-body">
        <form class="my-3" action="{{ route('admins.faqs.store') }}" method="post">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea name="answer" class="form-control @error('answer') is-invalid @enderror" cols="30" rows="5" required>{{ old('answer') }}</textarea>
                @error('answer')
                    <span class="form-error-message text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <button type="submit" class="btn btn-success">{{ __('Create FAQ') }}</button>
        </form>
    </div>
</x-dashboard.main-layout>
